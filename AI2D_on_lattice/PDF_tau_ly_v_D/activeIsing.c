...
int main(void)
{
  ...
  while(t<tmax) /* TEMPORAL LOOP */
      ...
      for(i=0 ; i<N ; i++)
	{
      ...
      /* Set magnetization threshold */
      if((t<2*lx/v)&&(fabs(tot_mag)>th_mag))
	th_mag=fabs(tot_mag);

      /* Detect reversal and output time elapsed since last reversal */
