...
int main(void)
{
  ...
  if(fscanf(f_input, "tgap = %lg tmax = %lg rho0 = %lg lx = %d ly = %d w0 = %lg beta = %lg eps = %lg Dx = %lg Dy = %lg", &tgap, &tmax, &rho0, &lx, &ly, &w0, &beta, &eps, &Dx, &Dy)!=10){exit(1);};

  N= (int) floor(rho0*lx*ly);
  dt=1./(2.*Dx+2.*Dy+w0*exp(beta));
  
  printf("dt = %lg tgap = %lg tmax = %lg rho0 = %lg N = %d lx = %d ly = %d w0 = %lg beta = %lg eps = %lg Dx = %lg Dy = %lg\n" , dt, tgap, tmax, rho0, N, lx, ly, w0, beta, eps, Dx, Dy);
